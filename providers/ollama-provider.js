...
export class OllamaProvider extends LLMProvider {
    ...
    async analyzeImage(imageData, prompt = "Explain what this image is about") {
        ...
        try {
            ...
            console.log(`Analyzing image with Ollama - media type: ${mediaType}, base64 length: ${imageData.base64.length}`);
            ...
            const response = await this.makeRequest('POST', '/api/chat', payload, {
            ...
            if (error.message.includes('CORS') || error.message.includes('403')) {
                throw new Error('CORS error: Please configure Ollama with OLLAMA_ORIGINS="chrome-extension://*"');
            } else if (error.message.includes('Connection refused') || error.message.includes('ECONNREFUSED')) {
                throw new Error('Cannot connect to Ollama service. Please ensure Ollama is running.');
            } else if (error.message.includes('timeout') || error.message.includes('ETIMEDOUT')) {
                throw new Error('Request timeout. Please try again or check Ollama service status.');
            ...
            } else if (error.message.includes('model not found') || error.message.includes('model') && error.message.includes('not found')) {
                throw new Error('Selected model does not support image analysis. Please use a vision model like llava.');
            } else {
